        ...
        <View style={styles.container}>
            ...
            <View style={styles.contentArea}>
              <View style={styles.inputField}>
                ...
                <TextInput style={styles.input} value={birthDay} onChangeText={setBirthDay}keyboardType='numeric'/>
              </View>
              <View style={styles.inputField}>
            ...
            <Button title='Calcular' onPress={() => setResult(calculateAge(`${birthYear}-${birthMonth}-${birthDay}`))}/>
            {result && <Text>Resultado: {result.split('-')[0]} {result.split('-')[0] != 1 ? 'anos' : 'ano'} {result.split('-')[1]} {result.split('-')[1] != 1 ? 'meses' : 'mês'} {result.split('-')[2]} {result.split('-')[2] != 1 ? 'dias' : 'dia'}</Text>}
        </View>
    )
}
